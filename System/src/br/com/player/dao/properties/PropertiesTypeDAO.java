...
public class PropertiesTypeDAO
  extends DAO
  implements Serializable
{
  ...
  public PropertiesType create(PropertiesType type, User user)
    throws NamingException, SQLException, Exception
  {
    try
    {
      ...
      this.preparedStatement = getConnection().prepareStatement("insert into properties_type (name, description, created, modified, user_id) values (?,?,?,?,?)", 1);
  ...
  public PropertiesType get(Long id)
    throws NamingException, SQLException, Exception
  {
    try
    {
      this.preparedStatement = getConnection().prepareStatement("select id, name, description, created, modified, user_id from properties_type where id = ?");
    ...
    throws NamingException, SQLException, Exception
  {
    try
    {
      this.preparedStatement = getConnection().prepareStatement("select id, name, description, created, modified, user_id from properties_type");
      this.resultSet = this.preparedStatement.executeQuery();
      list = new ArrayList();
      while (this.resultSet.next())
      {
