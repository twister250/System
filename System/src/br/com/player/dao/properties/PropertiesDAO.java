  ...
  implements Serializable
{
  ...
  public Properties create(Properties property, User user)
    throws SQLException, NamingException, Exception
  {
    try
    {
      ...
      this.preparedStatement = getConnection().prepareStatement("insert into properties (name, value, properties_type_id, created, modified, user_id) values (?,?,?,?,?,?)", 1);
    ...
    throws NamingException, SQLException, Exception
  {
    try
    {
      this.preparedStatement = getConnection().prepareStatement("select a.id, a.name, a.value, a.properties_type_id type_id, a.created, a.modified, a.user_id, b.name type_name, c.name user_name\tfrom properties a, properties_type b, user c\twhere a.id = ?\tand a.properties_type_id = b.id\tand a.user_id = c.id");
  ...
  public long update(Properties property)
    throws SQLException, NamingException, Exception
  {
    try
    {
      ...
      this.preparedStatement = getConnection().prepareStatement("update properties set name = ?, value = ?, properties_type_id = ?, modified = ? where id = ?");
      this.preparedStatement.setString(1, property.getName());
      this.preparedStatement.setString(2, property.getValue());
      this.preparedStatement.setLong(3, property.getType().getId().longValue());
      this.preparedStatement.setTimestamp(4, timestamp);
    ...
    throws SQLException, NamingException, Exception
  {
    try
    {
      this.preparedStatement = getConnection().prepareStatement("select a.id, a.name, a.value, b.id type_id, b.name type_name, a.created, a.modified,\ta.user_id, c.name user_name\tfrom properties a, properties_type b, user c\twhere a.properties_type_id = b.id and\ta.user_id = c.id");
      this.resultSet = this.preparedStatement.executeQuery();
      list = new ArrayList();
      while (this.resultSet.next())
      {
